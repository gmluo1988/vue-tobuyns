	...
	<div class="pagecontent">
		...
		<div class="pagecontent-body">
			...
			<el-table :data="reports" highlight-current-row v-loading="listLoading" @selection-change="selsChange" :row-class-name="tableRowClassName" style="width: 100%;">
				...
				<el-table-column prop="datachangeCreatetime" label="日期" :formatter="formatDate">
				</el-table-column>
			...
			<!--工具条-->
			<el-col :span="24" class="paginationtoolbar">
				</el-pagination>
...
<script>
	//引入BeBetterme的相关接口，供js调用
	import { getReportList, removeReport, batchRemoveReport, addReport, editReport,getSumScore } from '../../../api/api';
	//引入时间处理js文件
...
//				allScore,//总得分
		...
		methods: {
			...
			//列表中时间的时间戳显示转换（格式为：yyyy-MM-dd）
			formatDate: function (row, column) {
				let date = new Date(row.datachangeCreatetime)
				return formatDate(date, 'yyyy-MM-dd')
			},
			
			//获取总分
			getAllScore(){
				getSumScore().then((res)=>{
					this.totalScore=res;
				})
